...
to go
  ...
  [
    ...
    if not is-returning and (food > 0) ; if turtle over grass patch
    [
      ;print "food found"
      collect-food
...
to random-walk
  ifelse coin-flip?
    [
      right random max-turn-angle + 1
    ]
    [
      left random max-turn-angle + 1
    ]
